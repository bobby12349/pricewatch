...
const CORE_GOODS = [
  ...
  'Sugar', 'Cooking Oil', 'Ghee', 'Basmati Rice Broken', 'Potatoes', 'Onions',
  'Tomatoes', 'Mango Sindhri', 'Banana First'
];
...
function displayHistory(history) {
  ...
  const latestYear = sorted.at(-1).date.slice(0,4);
  const sameYear = sorted.filter(r => r.date.slice(0,4) === latestYear);
  const base = sameYear.length >= 2 ? sameYear : sorted;
...
function buildGroups() {
  ...
  state.grouped = [...map.values()].map(g => {
    ...
    g.displayHistory = displayHistory(g.history);
    g.latest = g.history[g.history.length - 1];
    const recent = g.displayHistory.length ? g.displayHistory : g.history;
    g.previous = recent.length > 1 ? recent[recent.length - 2] : (g.history.length > 1 ? g.history[g.history.length - 2] : null);
...
function renderTable() {
  ...
  tbody.innerHTML = state.filtered.map(g => `
    <tr data-key="${g.key}">
      ...
      <td>${sparkline(g.displayHistory || g.history, 112, 36)}</td>
    </tr>`).join('');
...
function sparkline(history, w=112, h=36) {
  ...
  const prev = history.length > 1 ? history.at(-2).price : history.at(-1).price;
  const stroke = history.at(-1).price > prev ? 'var(--red)' : history.at(-1).price < prev ? 'var(--green-2)' : 'var(--muted)';
...
function bigChart(history) {
  const hdata = displayHistory(history);
  if (!hdata.length) return '';
  const w = 620, h = 260, pad = 34;
  const pts = pointsFor(hdata, w, h, pad);
  const vals = hdata.map(r => r.price);
  const min = Math.min(...vals), max = Math.max(...vals);
  const d = pts.map((p,i) => `${i?'L':'M'}${p[0].toFixed(1)},${p[1].toFixed(1)}`).join(' ');
  const circles = pts.map((p,i) => `<circle cx="${p[0]}" cy="${p[1]}" r="5"><title>${niceDate(hdata[i].date)} — ${fmtRs(hdata[i].price)}</title></circle>`).join('');
  const labelEvery = hdata.length <= 7 ? 1 : Math.ceil(hdata.length / 6);
  const labels = hdata.map((r,i) => (i === 0 || i === hdata.length - 1 || i % labelEvery === 0) ? `<text x="${pts[i][0]}" y="248" text-anchor="middle">${r.date.slice(5).replace('-', '/')}</text>` : '').join('');
...
function renderDetail(g) {
  ...
  document.getElementById('detail').innerHTML = `
    ...
    <div class="meta-list">
      ...
      <div class="meta-row"><span>Graph points</span><strong>${displayHistory(g.history).length}</strong></div>
    </div>`;
}
